...
var ASM_CONSTS = {
  1887264: () => { if (typeof Module['_camerac'] === 'undefined') { var c = {}; c.devices = []; c.streams = {}; Module['_camerac'] = c; } },  
 1887387: ($0, $1, $2) => { var devPtr = $0; var w = $1; var h = $2; if (typeof Module['_camerac'] === 'undefined') { var c = {}; c.devices = []; c.streams = {}; Module['_camerac'] = c; } var vc = {}; vc.width = {ideal: w}; vc.height = {ideal: h}; var constraints = {}; constraints.video = vc; constraints.audio = false; navigator.mediaDevices.getUserMedia(constraints) .then(function(stream) { var video = document.createElement('video'); video.srcObject = stream; video.play(); var canvas = document.createElement('canvas'); canvas.width = w; canvas.height = h; var ctx = canvas.getContext('2d'); Module['_camerac'].stream = stream; Module['_camerac'].video = video; Module['_camerac'].canvas = canvas; Module['_camerac'].ctx2d = ctx; video.addEventListener('loadedmetadata', function() { Module['_camerac_emscripten_permission_outcome']( devPtr, 1, video.videoWidth, video.videoHeight); }); }) .catch(function(err) { console.error('[camerac] getUserMedia error:', err); Module['_camerac_emscripten_permission_outcome'](devPtr, 0, 0, 0); }); },  
 1888407: () => { const cam = Module['_camerac']; if (!cam) return; if (cam.stream) { cam.stream.getTracks().forEach(function(t) { t.stop(); }); } Module['_camerac'] = {}; },  
 1888565: ($0, $1, $2) => { const w = $0; const h = $1; const rgba = $2; const cam = Module['_camerac']; if (!cam || !cam.ctx2d || !cam.video) return 0; cam.ctx2d.drawImage(cam.video, 0, 0, w, h); const img = cam.ctx2d.getImageData(0, 0, w, h).data; HEAPU8.set(img, rgba); return 1; }
};
...
var _camerac_emscripten_permission_outcome = Module['_camerac_emscripten_permission_outcome'] = createExportWrapper('camerac_emscripten_permission_outcome');
...
var _mono_aot_camera_hardwareWeb_get_method = Module['_mono_aot_camera_hardwareWeb_get_method'] = createExportWrapper('mono_aot_camera_hardwareWeb_get_method');
...
var ___start_em_js = Module['___start_em_js'] = 1888883;
var ___stop_em_js = Module['___stop_em_js'] = 1903242;
