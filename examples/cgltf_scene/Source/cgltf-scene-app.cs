...
public static unsafe class CGLTFSceneApp
{
    ...
    static void CopyParserSceneToState(CGltfScene parserScene)
    {
        try
        {
            Info($"[MIGRATION] Copying parser scene data to state...");
            Info($"[MIGRATION] Parser has: Nodes={parserScene.NumNodes}, Meshes={parserScene.NumMeshes}, Primitives={parserScene.NumPrimitives}");
        ...
        Info($"[MIGRATION] Copied {state.scene.num_buffers} buffers");
        ...
        Info($"[MIGRATION] Copied {state.scene.num_images} images");
        ...
        Info($"[MIGRATION] Copied {state.scene.num_pipelines} pipelines");
        
        // Copy primitives
        Info($"[MIGRATION] About to copy {parserScene.NumPrimitives} primitives");
        state.scene.num_primitives = parserScene.NumPrimitives;
        for (int i = 0; i < parserScene.NumPrimitives; i++)
        {
            Info($"[MIGRATION] Copying primitive {i}...");
            var parserPrim = parserScene.Primitives[i];
            Info($"[MIGRATION] Got primitive reference, pipeline={parserPrim.PipelineIndex}, material={parserPrim.MaterialIndex}");
            ...
            // Initialize the buffer array if it's null
        ...
        Info($"[MIGRATION] Scene copy complete - Nodes: {state.scene.num_nodes}, Meshes: {state.scene.num_meshes}, Primitives: {state.scene.num_primitives}");
        }
        catch (Exception ex)
        {
            Error($"[MIGRATION] Exception in CopyParserSceneToState: {ex.Message}");
            Error($"[MIGRATION] Stack trace: {ex.StackTrace}");
        }
    }
    ...
    private static unsafe void Init()
    {
        ...
        Info($"[MIGRATION] Starting CGltfParser load of '{gltfFilePath}'");
        _parser.LoadFromFileAsync(gltfFilePath, 
            onComplete: (scene) => {
                Info($"[MIGRATION] CGltfParser load complete! Nodes: {scene.NumNodes}, Meshes: {scene.NumMeshes}, Materials: {scene.NumMaterials}");
                Info($"[MIGRATION] Buffers: {scene.NumBuffers}, Images: {scene.NumImages}, Pipelines: {scene.NumPipelines}");
                
                // Copy CGltfParser scene data to state.scene so existing rendering code works
                CopyParserSceneToState(scene);
                
                Info($"[MIGRATION] Scene data copied to state, rendering should now work!");
            },
            onFailed: (error) => {
                Error($"[MIGRATION] CGltfParser load failed: {error}");
                state.failed = true;
            });
    ...
    private static unsafe void Frame()
    {
        ...
        else
        {
            ...
            if (logThisFrame)
            {
                ...
                Info($"[CGLTF WORKING] Scene has {state.scene.num_nodes} nodes to render");
            }
            ...
            for (int node_index = 0; node_index < state.scene.num_nodes; node_index++)
            {
                ...
                for (int i = 0; i < mesh->num_primitives; i++)
                {
                    ...
                    if (logThisFrame)
                    {
                        Info($"[CGLTF WORKING] Drawing primitive {i}: pipeline={prim->pipeline}, material={prim->material}");
                        Info($"[CGLTF WORKING] Primitive: base_element={prim->base_element}, num_elements={prim->num_elements}, index_buffer={prim->index_buffer}");
                        Info($"[CGLTF WORKING] Vertex buffers: num={prim->vertex_buffers.num}");
                        for (int vb = 0; vb < prim->vertex_buffers.num; vb++)
                        {
                            int buf_idx = prim->vertex_buffers.buffer[vb];
                            var buf_state = sg_query_buffer_state(state.scene.buffers[buf_idx]);
                            Info($"[CGLTF WORKING]   VB[{vb}] -> buffer[{buf_idx}] id={state.scene.buffers[buf_idx].id}, state={buf_state}");
                        }
                        var idx_buf_state = sg_query_buffer_state(state.scene.buffers[prim->index_buffer]);
                        Info($"[CGLTF WORKING] Index buffer id={state.scene.buffers[prim->index_buffer].id}, state={idx_buf_state}");
                        Info($"[CGLTF WORKING] Pipeline id={state.scene.pipelines[prim->pipeline].id}");
                    }
                    ...
                    if (mat->is_metallic)
                    {
                        if (logThisFrame)
                        {
                            Info($"[CGLTF WORKING] Material texture indices: base_color={mat->metallic.images.base_color}, metallic_roughness={mat->metallic.images.metallic_roughness}");
                            Info($"[CGLTF WORKING]   normal={mat->metallic.images.normal}, occlusion={mat->metallic.images.occlusion}, emissive={mat->metallic.images.emissive}");
                            Info($"[CGLTF WORKING] Texture view IDs: images[0].tex_view.id={state.scene.images[0].tex_view.id}, images[1].tex_view.id={state.scene.images[1].tex_view.id}");
                        }
                    ...
                    if (logThisFrame)
                    {
                        Info($"[CGLTF WORKING] About to call sg_draw({prim->base_element}, {prim->num_elements}, 1)");
                    }
