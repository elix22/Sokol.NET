...
public unsafe class Texture : IDisposable
{
    ...
    unsafe void OnTextureLoaded(string filePath, byte[]? buffer, FileLoadStatus status)
    {
        ...
        {
            Info($"OnTextureLoaded: Texture file '{filePath}' loaded successfully, size: {buffer.Length} bytes");
            ...
            if (pixels == null)
            {
                Info($"OnTextureLoaded: Failed to decode texture file: {filePath}");
                return;
            }
        ...
        {
            Info($"OnTextureLoaded: Failed to load texture file: {status}");
        }

    }

    // TBD ELI , support GLTF
