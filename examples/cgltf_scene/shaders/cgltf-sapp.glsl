...
// DEBUG: All lighting functions commented out - just testing raw texture output
...
void main() {
    ...
    vec4 base_color = (has_base_color_tex > 0.5)
        ? texture(sampler2D(base_color_tex, base_color_smp), v_uv)
        : vec4(1.0, 0.0, 1.0, 1.0); // Magenta if no texture
    
    // Step 2: Get metallic/roughness (stored in Blue=metallic, Green=roughness channels)
    ...
    if (has_metallic_roughness_tex > 0.5) {
        ...
        roughness *= mr_sample.g; // Green channel = roughness
    }
    
    // Step 3: Get normal map (for now, just visualize it)
    vec3 normal = normalize(v_nrm);
    if (has_normal_tex > 0.5) {
        vec3 normal_sample = texture(sampler2D(normal_tex, normal_smp), v_uv).rgb;
        // Normal maps store normals in [0,1] range, need to convert to [-1,1]
        normal_sample = normal_sample * 2.0 - 1.0;
        // For now, just use the raw normal map values (we'll add proper tangent space later)
        normal = normalize(normal_sample);
    }
    ...
    if (has_emissive_tex > 0.5) {
        vec3 emissive_sample = texture(sampler2D(emissive_tex, emissive_smp), v_uv).rgb;
        emissive *= emissive_sample;
    }
    
    // DEBUG: Combine all textures for visualization
    // - Base color as primary color (95%)
    // - Normal map adds subtle directional tint (5% influence)
    // - Metallic darkens (shiny surfaces darker)
    // - Roughness brightens slightly (rough surfaces lighter)
    // - Occlusion darkens (crevices and corners darker)
    // - Emissive adds glow (added after occlusion)
    vec3 normal_vis = normal * 0.5 + 0.5;
    vec3 debug_color = base_color.rgb * (1.0 - metallic * 0.3) * (0.8 + roughness * 0.2);
    debug_color = debug_color * 0.95 + normal_vis * 0.05;
    debug_color *= occlusion; // Apply occlusion (darkens crevices)
    debug_color += emissive;   // Add emissive glow (not affected by occlusion)
    
    frag_color = vec4(debug_color, base_color.a);
}
@end
