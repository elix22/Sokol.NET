...
namespace Sokol
{
    ...
    public class TextureCache
    {
        ...
        /// </summary>
        public unsafe Texture2D GetOrCreate(string identifier, byte* data, int width, int height, sg_pixel_format format = sg_pixel_format.SG_PIXELFORMAT_RGBA8, SamplerSettings? samplerSettings = null)
        {
            ...
            var texture = new Texture2D(data, width, height, identifier, format, samplerSettings);
        ...
        /// </summary>
        public Texture2D? GetOrCreate(string identifier, byte[] data, sg_pixel_format format = sg_pixel_format.SG_PIXELFORMAT_RGBA8, SamplerSettings? samplerSettings = null)
        {
            // Include format in cache key to handle different formats of same texture
            string cacheKey = $"{identifier}_{format}";
            ...
            var texture = Texture2D.LoadFromMemory(data, identifier, format, samplerSettings);
            if (texture != null)
            {
                _cache[cacheKey] = texture;
            }
