...
// using System.Collections.LowLevel.Unsafe;
using Sokol;
...
namespace Mediapipe
{
  public class Image : MpResourceHandle
  {
    ...
    public Image(ImageFormat.Types.Format format, int width, int height, int widthStep, SharedBuffer pixelData, ImageFrame.Deleter deleter)
      : this(format, width, height, widthStep, pixelData.GetBufferPointer(), deleter)
    { }
    ...
    ///   </list>
    /// </remarks>
    public Image(ImageFormat.Types.Format format, int width, int height, int widthStep, SharedBuffer pixelData)
