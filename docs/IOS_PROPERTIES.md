...
**Note:** Once provided, the Team ID is cached locally in `~/.sokol-charp-cache/{projectName}.teamid` for convenience.
...
2. **Caching**: Team IDs are cached after first use. Delete `~/.sokol-charp-cache/{projectName}.teamid` to reset.
