# Interactive Android Install Script (Debug Configuration) - PowerShell
# Usage: interactive-android-install-debug.ps1 <project_path>
...
elseif ($deviceCount -eq 1) {
    ...
    & dotnet msbuild $ProjectPath -t:BuildAndroidDebugInstall -p:AndroidDeviceId="$device"
}
else {
    ...
    do {
        ...
        if ([int]::TryParse($selection, [ref]$selectionNum) -and $selectionNum -ge 1 -and $selectionNum -le $deviceCount) {
            ...
            Write-Host "🚀 Building and installing (Debug)..." -ForegroundColor Cyan
            & dotnet msbuild $ProjectPath -t:BuildAndroidDebugInstall -p:AndroidDeviceId="$selectedDevice"
            break
        }
        else {
