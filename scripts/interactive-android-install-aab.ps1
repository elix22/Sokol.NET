# Interactive Android AAB Install Script (PowerShell)
# Usage: interactive-android-install-aab.ps1 <project_path>
...
elseif ($deviceCount -eq 1) {
    ...
    Write-Host "🚀 Building AAB and installing..." -ForegroundColor Cyan
    & dotnet msbuild $ProjectPath -t:BuildAndroidInstall -p:AndroidDeviceId="$device" -p:SubTask="aab"
}
else {
    ...
    do {
        ...
        if ([int]::TryParse($selection, [ref]$selectionNum) -and $selectionNum -ge 1 -and $selectionNum -le $deviceCount) {
            ...
            Write-Host "🚀 Building AAB and installing..." -ForegroundColor Cyan
            & dotnet msbuild $ProjectPath -t:BuildAndroidInstall -p:AndroidDeviceId="$selectedDevice" -p:SubTask="aab"
            break
        }
        else {
