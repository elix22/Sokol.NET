...
# Interactive Android AAB Install Script
# Usage: interactive-android-install-aab.sh <project_path>
...
if [ "$device_count" -eq 0 ]; then
    ...
    echo "🚀 Building AAB and installing..."
    dotnet msbuild "$PROJECT_PATH" -t:BuildAndroidInstall -p:AndroidDeviceId="$device" -p:SubTask="aab"
else
    ...
    if [ -t 0 ] && [ -t 1 ]; then
        ...
        while true; do
            ...
            if [[ "$selection" =~ ^[0-9]+$ ]] && [ "$selection" -ge 1 ] && [ "$selection" -le "$device_count" ]; then
                ...
                echo "🚀 Building AAB and installing..."
                dotnet msbuild "$PROJECT_PATH" -t:BuildAndroidInstall -p:AndroidDeviceId="$selected_device" -p:SubTask="aab"
                break
            else
        ...
        if command -v osascript >/dev/null 2>&1; then
            ...
            selection=$(osascript -e "tell app \"System Events\" to display dialog \"Select Android Device:\" & return & \"$device_list\" default answer \"1\" with title \"Android Device Selection (AAB)\"" -e "text returned of result" 2>/dev/null)
