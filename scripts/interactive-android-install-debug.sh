...
# Interactive Android Install Script (Debug Configuration)
# Usage: interactive-android-install-debug.sh <project_path>
...
if [ "$device_count" -eq 0 ]; then
    ...
    echo "🚀 Building and installing (Debug)..."
    dotnet msbuild "$PROJECT_PATH" -t:BuildAndroidDebugInstall -p:AndroidDeviceId="$device"
else
    ...
    if [ -t 0 ] && [ -t 1 ]; then
        ...
        while true; do
            ...
            if [[ "$selection" =~ ^[0-9]+$ ]] && [ "$selection" -ge 1 ] && [ "$selection" -le "$device_count" ]; then
                ...
                echo "🚀 Building and installing (Debug)..."
                dotnet msbuild "$PROJECT_PATH" -t:BuildAndroidDebugInstall -p:AndroidDeviceId="$selected_device"
                break
            else
        ...
        if command -v osascript >/dev/null 2>&1; then
            ...
            selection=$(osascript -e "tell app \"System Events\" to display dialog \"Select Android Device (Debug Build):\" & return & \"$device_list\" default answer \"1\" with title \"Android Device Selection\"" -e "text returned of result" 2>/dev/null)
